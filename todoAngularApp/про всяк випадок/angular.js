...
var app = angular.module('todoApp', ['ngRoute']);
    ...
    app.constant('todoApp', {url: '127.0.0.1:80'})
    ...
    app.controller('mainController', function($location, $rootScope, $http, $scope, $route, $routeParams, $location) {
        ...
        $scope.submitForm = function() {
            ...
            if ($scope.userForm.$valid) {
                $http({
                    method: 'POST',
                    url: 'http://symfonytest.loc/app_dev.php/registration/?firstName='+$scope.name+'&email='+$scope.email+'&username='+$scope.username+'&pass='+$scope.password
    ...
    app.controller('loginController', function($location, $window, $rootScope, $http, $scope, $route, $routeParams, $location) {
        ...
        $scope.submitForm = function() {
            ...
            if ($scope.userForm.$valid) {
                $http({
                    method: 'GET',
                    url: 'http://symfonytest.loc/app_dev.php/login/?username='+$scope.username+'&pass='+$scope.password
                }).then(function (response){                    
                    console.log(response);
                    $rootScope.token = response.data['token'];
                    $rootScope.userId = response.data['userId'];
    ...
    app.controller('TodoListController', function($location, $rootScope, $http, $scope, $route, $routeParams, $location) {
        ...
        if ($rootScope.token===''){
            ...
            $http({
                    method: 'GET',
                    url: 'http://symfonytest.loc/app_dev.php/user/'+$rootScope.userId,
                    headers: {
        ...
        todoList.addTodo = function() {                
            ...
            $http({
                    method: 'POST',
                    url: 'http://symfonytest.loc/app_dev.php/user/'+$rootScope.userId+'/add/?text='+todoList.todoText,
                    headers: {
                                'X-AUTH-TOKEN': $rootScope.token
                             }
                }).then(function (response){                    
        ...
        todoList.archive = function() {
          ...
          angular.forEach(oldTodos, function(todo) {
            if (!todo.done){                
                ...
                $http({
                    method: 'POST',
                    url: 'http://symfonytest.loc/app_dev.php/user/'+$rootScope.userId+'/dell/'+url,
                    headers: {
                                'X-AUTH-TOKEN': $rootScope.token
                             }
                }).then(function (response){                    
